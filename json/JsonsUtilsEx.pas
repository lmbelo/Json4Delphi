    ...
    begin
      ...
      case pl[i]^.PropType^.Kind of
        ...
        begin
          ...
          {$IFNDEF FPC}
          lpTypeInfo :=  PPPTypeInfo(lTypeData^.BaseType);
          ...
          case lTypeInfoFPC^.Kind of
            tkAString,
          {$ENDIF}
            tkString : //Warning, take care of {$IFDEF} just upside :)
            begin
...
begin
  ...
  try
    ...
    if Not(lJSON.StructType = jsObject) then
    begin
    ...
    begin
      ...
      case lpl[i]^.PropType^.Kind of
        ...
        tkClass:
        begin
          if (lJsValue.ValueType = jvObject) or (lJsValue.ValueType = jvNone) then
          begin
            //In jvNone case, we do nothing (JSON has not this property, but it is object which driven our build.
            if (lJsValue.ValueType = jvObject) then
            begin
        ...
        tkDynArray :
        begin
          if lJsValue.ValueType = jvArray then
          begin
